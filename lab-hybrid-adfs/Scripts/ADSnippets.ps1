...
function Set-O365AzureDNS 
{
    ...
    #MX Records
    $Record    = New-AzDnsRecordConfig -Exchange $MX.value -Preference $MX.ttl
    $MXRecord  = New-AzDnsRecordSet -Name $MX.name -RecordType MX -ResourceGroupName $RGName -TTL $MX.ttl -ZoneName $ZoneName -DnsRecords $Record -Overwrite -Force -Verbose

    #CNAME Records
    foreach($rec in $CNAME) {
        $Record       = New-AzDnsRecordConfig -Cname $rec.value
        $CNameRecord  = New-AzDnsRecordSet -Name $rec.name -RecordType CNAME -ResourceGroupName $RGName -TTL $rec.ttl -ZoneName $ZoneName -DnsRecords $Record -Overwrite -Force -Verbose
    }

    #TXT Records
    $Record    = New-AzDnsRecordConfig -Value $TXT.value
    $TXTRecord = New-AzDnsRecordSet -Name $TXT.name -RecordType TXT -ResourceGroupName $RGName -TTL $TXT.ttl -ZoneName $ZoneName -DnsRecords $Record -Overwrite -Force -Verbose

    #SRV Records
    foreach($rec in $SRV) {
        $Record    = New-AzDnsRecordConfig -Priority $rec.priority -Weight $rec.weight -Port $rec.port -Target $rec.target
        $SRVRecord = New-AzDnsRecordSet -Name "$($rec.service).$($rec.protocol)" -RecordType SRV -ResourceGroupName $RGName -TTL $rec.ttl -ZoneName $ZoneName -DnsRecords $Record -Overwrite -Force -Verbose
    }
}